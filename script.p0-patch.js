...
// ========================================
const PROJECT_ORDER = [
    'work-old-gray-barn-celebration.html', // July 28, 2026
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- Homepage Work Preview — pinned editorial picks (no lottery) ---
    function initWorkPreview() {
        const grid = document.querySelector('.work-preview__grid');
        if (!grid) return;

        // When markup is already authored (data-pinned), leave it alone.
        const section = document.querySelector('.work-preview');
        if (section && section.getAttribute('data-pinned') === 'true') return;

        const pinnedCases = [
            {
                href: 'work-old-gray-barn-celebration.html',
                src: 'assets/old-gray-barn-celebration-01.jpg',
                alt: 'Celebration of Life production at The Old Gray Barn in Dorset, Vermont.',
                number: 'Jul 2026',
                title: 'A Celebration of Life where every voice in the barn was heard.',
                venue: 'The Old Gray Barn \u2014 Dorset, VT'
            },
            {
        ...
        grid.innerHTML = pinnedCases.map(c => `
    ...
    function initTestimonialRotator() {
        ...
        // Start on the pinned Hildene quote (authored edit), then rotate
        currentIdx = 0;
    ...
    // --- Story Beat — pinned first story; gentle rotation through curated set ---
    function initStoryBeatRotator() {
        ...
        // Markup already holds the authored first story when data-pinned
        if (section.getAttribute('data-pinned') === 'true') {
            // Still allow gentle rotation through a curated set after first paint
        }
        ...
        const stories = [
            ...
            {
                text: "A Celebration of Life at The Old Gray Barn in Dorset. Three 75-inch monitors, four speakers, eighteen uplights, a Sunday rehearsal with the family, and a polished recording of every voice in the room.",
                eyebrow: "The Old Gray Barn \u2014 Dorset, VT \u2014 July 2026",
                href: "work-old-gray-barn-celebration.html"
            },
            {
        ...
        // Always start at the pinned Equinox story (index 0) — no random lottery
        let idx = 0;
    ...
    // --- Hero Video Fade-In ---
    // Keep graded poster until canplay so heroes never flash an ink void.
    // Poster stays on the element; CSS can still fade the video in when ready.
    ...
    // --- Homepage Insights teasers — pinned Quiet Room field notes (no lottery) ---
    function selectPinnedInsights() {
        const insightsGrid = document.querySelector('.insights-highlight-grid');
        if (!insightsGrid) return; // Not on a page with the grid
        
        // Curated field notes — venue/cue/failure-mode specificity, not SEO mill voice
        const pinnedArticles = [
            {
                url: 'article-wedding-av-equipment.html',
                image: 'assets/insights-wedding-av.jpg',
                category: 'Weddings',
                title: 'What the vows need before the first dance',
                description: 'Ceremony mics, reception fills, and the backup path when weather moves the timeline — gear choices that keep every seat in the tent hearing clearly.',
            ...
            },
            {
                ...
                category: 'Production',
                title: 'Questions worth asking before you book AV',
                description: 'Who owns the run-of-show? What fails first outdoors? How do they talk to your planner? The difference between a production partner and a truck of gear.',
                cta: 'Read field notes'
            },
            {
                ...
                category: 'Show day',
                title: 'Why rehearsal time is non-negotiable',
                description: 'A quiet hour with the actual room — walk-in music, toast order, first-dance cue — is how missed moments stay off the guest list.',
                cta: 'Read field notes'
            }
        ];
        
        insightsGrid.innerHTML = pinnedArticles.map(article => `
    ...
    // Run immediately (no-op when homepage grid is removed)
    selectPinnedInsights();
...
function initLightbox() {
    ...
    lightbox.innerHTML = `
        ...
        <div class="lightbox-content">
            <img class="lightbox-image" src="assets/logo-wordmark.png" alt="Gallery image">
        </div>
...
function initEqCaseLightbox() {
    ...
    lb.innerHTML =
        ...
        '<div class="eq-lightbox__img-wrap"><img class="eq-lightbox__img" src="assets/logo-wordmark.png" alt="Gallery image"></div>' +
