...
function initAccessibilityMenu() {
    ...
    accessToggle.id = 'accessibility-menu-toggle';
    accessToggle.setAttribute('aria-label', 'Accessibility options');
    accessToggle.innerHTML = `
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            <circle cx="12" cy="8" r="2"></circle>
            <path d="M12 14c-3.5 0-6.5 1.5-6.5 4v2h13v-2c0-2.5-3-4-6.5-4z"></path>
            <path d="M7 12h10M12 12v8"></path>
        </svg>
    `;
    document.body.appendChild(accessToggle);
    ...
    accessMenu.innerHTML = `
        ...
        <div class="accessibility-option">
            ...
            <input type="range" id="text-size" min="90" max="130" value="100" step="10">
        </div>
    `;
    ...
    accessToggle.addEventListener('click', () => {
        accessMenu.classList.toggle('visible');
    });

    // Close menu when clicking outside
    document.addEventListener('click', (e) => {
        if (!accessMenu.contains(e.target) && !accessToggle.contains(e.target)) {
            accessMenu.classList.remove('visible');
        }
    });
    ...
    // Accessibility options event listeners
    document.getElementById('reduce-motion').addEventListener('change', (e) => {
        document.documentElement.style.setProperty('--animation-speed', e.target.checked ? '0' : '1');
...
function loadAccessibilityPreferences() {
    ...
    // Text size
    const textSize = localStorage.getItem('text-size') || '100';
...
function initLiveChat() {
    // Create a simple chat button that opens contact form or email
    // (Replace with actual Tawk.to code when you have an account)
    ...
    chatBtn.style.cssText = `
        ...
        width: 60px;
        height: 60px;
        background: linear-gradient(135deg, var(--color-accent), #5577ff);
        color: white;
        border: none;
        border-radius: 50%;
        cursor: pointer;
    ...
    chatBtn.addEventListener('click', () => {
        // Open contact page or show chat widget
        window.location.href = '/contact.html';
    });
    
    chatBtn.addEventListener('mouseenter', () => {
        chatBtn.style.transform = 'scale(1.1)';
        chatBtn.style.boxShadow = '0 6px 25px rgba(65, 105, 225, 0.5)';
    });
    
    chatBtn.addEventListener('mouseleave', () => {
        chatBtn.style.transform = 'scale(1)';
        chatBtn.style.boxShadow = '0 4px 20px rgba(65, 105, 225, 0.4)';
    });
    ...
    style.textContent = `
        @keyframes pulse {
            ...
            50% {
                box-shadow: 0 4px 20px rgba(65, 105, 225, 0.4), 0 0 0 10px rgba(65, 105, 225, 0.1);
            }
        }
...
/* 
 * TO USE TAWK.TO LIVE CHAT:
 * 1. Sign up at https://www.tawk.to/
 * 2. Get your property ID
 * 3. Replace the initLiveChat function with:
 * 
 * function initLiveChat() {
